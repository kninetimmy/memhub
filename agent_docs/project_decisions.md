...
## 2026-05-12 - Evaluating memhub-primary is staged behind parser-fix evidence

- A working-session analysis report (local, not committed) identifies six bootstrap parser bugs in `src/commands/bootstrap_k9.rs` and a 12-gap roadmap for a full memhub-primary replacement of the K9 framework. The directional shift to memhub-primary is *not* committed.
- The evaluation plan lives in `docs/roadmap/memhub-primary-evaluation.md`. Phase 1 lands the parser fixes (`M6-001`, `M6-002`); Phase 2 re-runs `bootstrap-k9` on Free-AI-SSD and captures findings (`M6-003`); Phase 3 routes to one of three outcomes — commission a separate `memhub render` design doc, extend the parser further, or close the evaluation with memhub staying complementary.
- PRD §2 ("markdown files stay as the entry point"), PRD §4 non-goals, and `docs/roadmap/k9-integration.md` non-goals (no bidirectional sync, no DB mapping of `project_state.md` / `project_arch.md`) all remain in force. None are modified by this evaluation.
- Reasoning: the analysis report's own honest-take is that replacement is genuinely better only if the parser fix lands cleanly *and* a `memhub render` materializes acceptably. Committing the roadmap to a directional shift before the parser fix gives us evidence about post-fix bootstrap quality would be premature.

## 2026-05-12 - K9 canonical conventions (H3 backlog items, em-dash decisions) are the parser target

- `K9-Claude-Framework/docs/file-structure.md:156-208` is the authoritative K9 framework spec. It documents `project_backlog.md` items as `### Title` H3 delimiters with bulleted bolded-field bodies (`- **Scope.** ...`, `- **Affected files.** ...`, `- **Status.** triaged | planning | in-progress | blocked | done`), and `project_decisions.md` entries as `## YYYY-MM-DD — Title` with a Unicode em-dash separator (U+2014).
- `M6-001` and `M6-002` target only this canonical format. Free-AI-SSD's K9 history follows it; memhub's own `agent_docs/` does not.
- memhub's own `agent_docs/project_backlog.md` and `project_decisions.md` will be migrated to canonical structural delimiters via `M6-004` (em-dash decision separator, H3 backlog headings, preserving the existing prose `Notes:` paragraphs rather than decomposing them into K9's bulleted slot vocabulary). This closes the dogfood gap: memhub claims to integrate with K9 and should use K9's documented format in its own continuity files.
- The "lenient: support both formats" option was rejected because doubling the parser's surface area to permit memhub's own divergence would bake that divergence into the supported contract, which is the wrong direction. The narrower bullet-fallback for the trailing `Status: done` line is retained only as a tolerated input recognition path, not as a supported authoring convention.

## 2026-05-12 - K9 framework deprecation: memhub becomes primary durable store

- The end-state has memhub as the single durable source of project memory across Codex and Claude Code; K9's slash commands, markdown templates, and routing brain are retired.
