...
M8 (SQL+RAG hybrid recall) is half shipped. PR1, PR2, and PR3 landed
this session on main in three atomic commits:

- 3168c8c — PR1: BGE-small-en-v1.5 bundled via build.rs auto-download
  (SHA256-pinned) and exposed through src/retrieval/embeddings.rs
  with fastembed's UserDefinedEmbeddingModel and CLS pooling.
- cd1ae3f — PR2: migration 0009 adds the embeddings table and
  contentless FTS5 virtual tables over facts/decisions/tasks, with
  AI/AU/AD sync triggers and a one-shot rebuild backfill.
- 8d2c59f — PR3: new [retrieval] mode = fts | hybrid config toggle;
  fact/decision/task add paths (including review::accept) eager-embed
  inside the source-write transaction when mode = hybrid; migration
  0010 cascades source-row deletes into embeddings via triggers.

Test suite at 204 green (was 175 entering the session). Default fts
mode keeps the model unloaded so most tests pay no embedding cost.

## Next up

PR4–PR6 remain, in order:

1. PR4: `memhub recall` CLI + `memhub.recall` MCP tool with hybrid
   scoring (0.5 FTS + 0.5 vector + stale penalty; FTS-only when
   mode = fts). Filters: --source-type, --max-results, --json,
   --include-stale, --accepted-only. Empty result returns empty bundle.
2. PR5: `/recall` and `/reindex` Claude/Codex skills; CLAUDE.md
   Session Continuity rule teaching agents to prefer recall over
   reading the ledger.
3. PR6: tests/retrieval_golden.json + `memhub eval retrieval` +
   `/eval-recall` skill. Acceptance: Recall@3 baseline reported.

Also pending before any external consumer sees M8: reinstall the
memhub binary on PATH. ~/.cargo/bin/memhub and the ~/.local/bin
shadow both predate this session and still report schema 0008, so
any agent invoking MCP or CLI from outside this repo misses the new
embeddings/FTS surface.
...
- M8-specific:
  - Per-result token estimates in the recall response? (PR4 call.)
...
_Last updated 2026-05-13 21:49:24 by claude:wrap-up._
...
- `src/config/` - per-repo config model and read/write helpers, including `[render].output_dir` and `[retrieval].mode`
...
- `src/retrieval/` - bundled BGE-small embedding wrapper (`embeddings.rs`) and the eager-embed write-path helper (`persist.rs`)
...
- Broader indexed retrieval beyond exact file history, decision FTS, latest-command lookup, the contentless FTS5 over fact/decision/task source tables (PR2), the embeddings BLOB (PR3), and the narrow MCP read tools is deferred until a real workflow demands it. Hybrid scoring and the `recall` surface itself are PR4 (M8) and not yet shipped.
...
- Switching `[retrieval].mode` from `fts` to `hybrid` on a populated DB requires a one-shot `memhub index rebuild` to backfill embeddings for pre-existing rows. That command is not implemented yet (PR4 territory).
...
_Last updated 2026-05-13 21:50:12 by claude:wrap-up._
