...
| W3 | **Transcript mode — before issue #214:** copy Claude/Codex session JSONL to `.memhub/transcripts/<date>-<session-id>.jsonl.zst` + one pointer row in a new `session_transcripts` table (`CREATE TABLE IF NOT EXISTS`), reusing `session_scraper`'s dir-resolution and session-id mapping. **After issue #214:** also request complete OpenCode exported JSON and store it as `.json.zst` through the same pointer/retention/isolation lifecycle; this does not add OpenCode to `session_scraper`. **Every agent's archive remains never embedded, never recalled, and excluded from export** (mirrors the M11 isolation rule). Retention via `[wrap_up] transcript_retention_days` | ~150 lines before #214 | Transcripts can contain secrets — deny-list filters paths, not content; §11 Q8. Disk growth: zstd + retention |
