...
- **Run C (2026-07-17–18, PRs #151–#156): audit remediation and CI baseline.** The maintenance-on-open contract is now codified with tests (task 120, #151). Initial CI landed: Windows/macOS build+test lanes, a `--features viz` lane, hash-keyed model-asset caching, an ubuntu fmt+clippy lint gate, and a weekly scheduled `cargo audit` (task 115, #152; lint gate extended by #156). The `sync_md` channel is retired, with `memhub upgrade` now purging stale rendered CLAUDE/AGENTS twins (task 119, #153). A small-hardening batch closed four known issues: a corrupt-embedding staleness warning replacing silent FTS-only degradation, the `memhub audit md` panic fix, an import `writes_log` guard, and atomic export (task 117, #154). A docs/roadmap reconciliation pass followed (task 118, #155), and a mechanical `rustfmt` + `clippy` adoption pass closed out the CI lint gate (task 116, decision 160, #156).
