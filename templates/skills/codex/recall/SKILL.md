...
This is the Codex counterpart to the Claude Code `/recall` skill.
Both call into the same `memhub recall` CLI and `memhub.recall` MCP
tool; they differ only in the agent identifier on whatever read-side
...
Prefer the `memhub.recall` MCP tool when it's available — it returns
structured JSON directly and avoids shell quoting.

```
memhub.recall(query="<one-line natural-language question>")
```

CLI fallback (no MCP, or you want to pipe the bundle to other shell
tools):
