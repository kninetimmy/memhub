...
host-aware optional step at the end offers `AGENTS.md` by default
(since this is Codex); the same flow under Claude Code offers
`CLAUDE.md`.
...
memhub does not write `AGENTS.md` or `CLAUDE.md` itself. After render,
check whether one exists at repo root:

- **Neither exists.** Offer to write a minimal `AGENTS.md` pointing at
  the rendered files for session continuity (Codex reads `AGENTS.md`
  by convention; offer `CLAUDE.md` with identical body if the user
  also uses Claude Code on this repo). Template:
  ...
  When writing to memhub from this Codex CLI, pass
  `--source user+agent:codex` on `memhub fact add` and
  `memhub decision add` that go through `/wrap-up` (agent surfaced,
  user approved). Direct CLI writes the user types themselves take
  the `user` default.
