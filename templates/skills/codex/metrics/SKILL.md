---
name: metrics
description: Show the token-accounting dashboard panel for the current memhub project — 7-day and 30-day recall/session token totals, context offset vs full-ledger baseline, and a recent-sessions table.
framework: memhub
...
This is the Codex counterpart to the Claude Code `/metrics` skill.
Both call into the same `memhub.metrics` MCP tool and `memhub metrics status`
CLI; they differ only in the agent identifier on whatever read-side
telemetry the host captures.
