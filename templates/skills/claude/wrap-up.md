...
8. **Stale-fact re-verify candidates.** Run `memhub fact list --json`
   ...
   probably wrong; tell me so I can fix or remove it instead of
   verifying it.
...
# 3. New decisions
memhub decision add "<title>" --rationale "<rationale>" --source user+agent:claude-code --json --actor claude:wrap-up
...
- That the rendered files are local generated output and are not meant
  to be committed unless this repo explicitly opts into a tracked
  render path.
