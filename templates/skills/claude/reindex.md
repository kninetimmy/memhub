...
4. **Run rebuild.**
   ...
   ```bash
   memhub index rebuild --actor <agent>:reindex --json
   ```

   The actor string should identify the agent and the operation
   (e.g. `claude-code:reindex`) for the writes_log audit row.
...
- For the equivalent CLI gesture from Codex, see the
  `/reindex` skill under `templates/skills/codex/`.
